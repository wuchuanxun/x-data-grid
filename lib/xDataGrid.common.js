...
// CONCATENATED MODULE: ./node_modules/@babel/runtime/helpers/createForOfIteratorHelper/_index.mjs
...
// CONCATENATED MODULE: ./packages/dots.vue?vue&type=script&lang=js&
 /* harmony default export */ var packages_dotsvue_type_script_lang_js_ = (dotsvue_type_script_lang_js_); 
...
// CONCATENATED MODULE: ./packages/dots.vue
...
var component = normalizeComponent(
  packages_dotsvue_type_script_lang_js_,
  dotsvue_type_template_id_56593a9a_render,
  staticRenderFns,
  false,
  null,
  null,
...
// CONCATENATED MODULE: ./packages/x-data-grid.js
...
var objectTakeByKey = function objectTakeByKey(obj, key) {
  if (key) {
...
var objectSetByKey = function objectSetByKey(obj, key, value) {
  var keys = key.split('.');

  if (keys.length > 1) {
    var objAt = keys.slice(0, -1).reduce(_set, obj);
...
/* harmony default export */ var x_data_grid = ({
  name: 'x-data-grid',
  props: {
    // 表头列名称
    columns: {
  ...
  render: function render(createElement) {
    ...
    fixR = fixRAll;
    var renderData = this.filterSource; // 创建标题
    ...
    this.columns.forEach(function (col, index) {
      ...
      var titleContent = [col.title];

      if (col.type === '_check') {
        titleContent = [createElement('input', {
          attrs: {
            type: 'checkbox',
            checked: false
          },
          on: {
            change: function change(e) {
              ...
              var _iterator = _createForOfIteratorHelper(renderData),
                  _step;
      ...
      if (col.sortable) {
        event.click = function (e) {
          ...
          that.columns.forEach(function (p, idx) {
            if (idx === index) {
          ...
          if (that.sortType !== 'normal') {
            var dir = that.sortType === 'asc' ? 1 : -1;
            that.filterSource.sort(function (a, b) {
      ...
      if (col.fixed === 'left') {
        ...
        }, titleContent), createElement('div', {
          style: {
            top: 0,
    ...
    if (this.loading) {
      // 渲染替代文本
      trs.push(createElement('tr', [createElement('td', {
        attrs: {
      ...
      var preGroup;
      renderData.forEach(function (row, rindex) {
        // 遍历行
        if (row._group !== preGroup) {
          // 渲染一个组
          var active = !that.$data._deActiveGroup.includes(row._group);
          var groupName = row._group;

          if (that.showGroupQty) {
            var qty = renderData.filter(function (p) {
              return p._group === groupName;
            }).length;
            groupName += " [".concat(qty, "\u9879]");
          }
            ...
            on: {
              click: function click(e) {
                if (active) {
                  ...
                  // 移除当前
                  var idx = that.$data._deActiveGroup.indexOf(row._group);
          ...
          var _active = !that.$data._deActiveGroup.includes(row._group);
        ...
        fixL = 0;
        fixR = fixRAll;
        that.columns.forEach(function (cell) {
          // 遍历单元格
          var styleText = cell.ellipsis ? {
            textOverflow: 'ellipsis',
            whiteSpace: 'nowrap',
            overflow: 'hidden'
          } : {};
          var tdDirectives = cell.ellipsis ? {
            directives: [{
              name: 'tooltip'
            }]
          } : {};
          var tdContent = objectTakeByKey(row, cell.key);
          var originalValue = tdContent;
          var concatAttrs = {};
          ...
          if (cell.type === 'number') {
            if (tdContent === undefined || tdContent === null) {
              ...
              tdContent = numeral_default()(tdContent).format(cell.format);
            }
          } else if (cell.type === 'text' && tdContent instanceof Object) {
            tdContent = JSON.stringify(tdContent);
          }

          var editIt = that.editable;

          if (cell.type === '_check') {
            tdContent = [createElement('input', {
              attrs: {
                type: 'checkbox'
              },
              domProps: {
              ...
              on: {
                change: function change(e) {
                  ...
                  var _iterator2 = _createForOfIteratorHelper(renderData),
                      _step2;
          ...
          if (!editIt) {
            tds.push(createElement('td', _objectSpread2({
              attrs: concatAttrs,
              class: cell.class,
          ...
          on: {
            click: function click() {
              if (that.clickExpand) {
                ...
                var _iterator3 = _createForOfIteratorHelper(renderData),
                    _step3;
    ...
    if (this.pageSize > 0) {
      var pages = Math.ceil(this.filterSource.length / this.pageSize);
    ...
    var navigationDom = createElement('span', {
      style: {
        float: 'right'
      }
    }, navigation);
    ...
    if (!this.hiddenInfo) {
      var t = "\u5171".concat(this.$data._source.length, "\u6761\u6570\u636E");

      if (searchOptions.length > 0 && this.filterSource.length !== this.$data._source.length) {
        t += "\uFF0C\u7B5B\u9009\u7ED3\u679C".concat(this.filterSource.length, "\u6761");
      }

      if (this.$data._checkedKeys.length > 0) {
        t += "\uFF0C\u5DF2\u9009".concat(this.$data._checkedKeys.length, "\u6761");
      }
    ...
    if (this.overwriteSearch) {
      ...
      }, '包含'), createElement('option', {
        attrs: {
          value: '!'
        }
      }, '不包含'), createElement('option', {
        attrs: {
          value: '^'
        }
      }, '开头为'), createElement('option', {
        attrs: {
          value: '='
        }
      }, '等于')]));
      search.push(createElement('input', {
        ...
        on: {
          'input': Object(debounce["debounce"])(function (e) {
            ...
            if (text.length === 0) {
              ...
              var fuse = new fuse_esm(that.$data._source, that.options);
              that.filterSource = fuse.search(that.options.prefix + text).map(function (p) {
                return p.item;
              });
            }
    ...
    var nodes = this.hiddenSearch ? [] : [createElement('div', {
      class: 'x-table-title',
      style: {
        textAlign: 'left',
        display: 'flex'
    ...
    }, [createElement('colgroup', cols), createElement('thead', [createElement('tr', ths)]), createElement('tbody', trs)])]));
  ...
  watch: {
    source: function source() {
      ...
      this.$data._checkedKeys = this.$data._source.filter(function (p) {
        return p._checked;
      }).map(function (p) {
        return p[rowKey];
      });
      ...
      if (this.searchSchema.trim() === '') {
        ...
        var fuse = new fuse_esm(this.$data._source, this.options);
        this.filterSource = fuse.search(this.options.prefix + this.searchSchema).map(function (p) {
          return p.item;
        });
      } // 重新排序
      ...
      if (this.sortType !== 'normal') {
        var dir = this.sortType === 'asc' ? 1 : -1;
        var that = this;
        this.filterSource.sort(function (a, b) {
          return defaultSortFn(a._group, b._group) * 10 + that.sortFn(a[that.sortKey], b[that.sortKey]) * dir;
        });
      } else {
        this.filterSource.sort(function (a, b) {
  ...
  created: function created() {
    ...
    this.columns.map(function (col, index) {
      ...
      col._sortType = col._sortType || 'normal'; // 新建字段，标识当前列在数组中的索引

      col._index = index; // 数值的格式化
    ...
    this.filterSource = this.$data._source; // 默认排序
    ...
    this.filterData = Object(debounce["debounce"])(this.filterData, 300);
  },
  methods: {
    filterData: function filterData(handler) {
      if (typeof handler !== 'function') return;
      this.filterSource = handler(this.$data._source);
    }
  }
});
// EXTERNAL MODULE: ./packages/x-data-grid.scss
var packages_x_data_grid = __webpack_require__("883e");
...
x_data_grid.install = function (Vue) {
  return Vue.component(x_data_grid.name, x_data_grid);
};

var packages_install = function install(Vue) {
  Vue.component(x_data_grid.name, x_data_grid);
};
/* 支持使用标签的方式引入 */
...
/******/ });
//# sourceMappingURL=xDataGrid.common.js.map